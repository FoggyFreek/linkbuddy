...
const StatsPanel = lazy(() => import('../../features/editor/components/StatsPanel.js'))
import EditorHeader from '../../features/editor/components/EditorHeader.js'
import PageSwitcher from '../../features/editor/components/PageSwitcher.js'
...
export default function Editor() {
  ...
  const [preview, setPreview] = useState<ResolvedPage | null>(null)
  ...
  const adoptPage = useCallback((loaded: EditorPage) => {
    setPage(loaded)
  ...
  const currentSession = () => {
    if (!sessionRef.current) throw new Error('Editor session is unavailable')
  ...
  const selectPage = async (pageId: number) => {
    ...
    await flushSave()
    try {
      adoptPage(await getEditorPage(currentSession(), pageId))
    } catch (err) {
  ...
  const setTheme = (theme: DraftTheme) => {
  ...
  const publish = async () => {
    await flushSave()
    try {
      const result = await publishPage(currentSession(), page.id)
  ...
  const openPreview = async () => {
    await flushSave()
    try {
      setPreview(await getPreview(currentSession(), page.id))
      setTab('preview')
    } catch (err) {
      setSaveState(saveErrorState(err))
    }
  ...
  const refresh = async () => {
    try {
      const loaded = await refreshContent(currentSession(), page.id)
      setPage(loaded)
    } catch {
