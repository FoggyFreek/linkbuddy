...
const CHANNELS: ShareChannel[] = [
  {
    ...
    label: 'WhatsApp',
    Icon: WhatsAppIcon,
    href: (url: string, text: string) => `https://wa.me/?text=${encodeURIComponent([text, url].join(' '))}`,
  },
  {
    id: 'facebook',
    label: 'Facebook',
...
export default function ShareButton({ url, title, onShare = () => {}, variant = 'floating' }: {
  ...
  // so this is empty and the menu uses MUI's default container (the app scheme).
  ...
  const variantSx: SxProps<Theme> | undefined = {
    floating: { position: 'fixed', ...pinnedSx },
    corner: { position: 'absolute', ...pinnedSx },
  }[variant as 'floating' | 'corner']
