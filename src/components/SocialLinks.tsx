...
import type { Band, LinkClickHandler } from '../types.js'

const SOCIAL_ICONS: Record<string, ComponentType<{ size: number; mono?: boolean }>> = {
  ...
  return (
    ...
    >
      {items.map((s) => {
        ...
        const handle = band?.socials?.[s.key]
        if (!Icon || !handle) return null
        return (
          <IconButton
            key={s.key}
            component="a"
            href={socialHref(s, handle)}
            target="_blank"
